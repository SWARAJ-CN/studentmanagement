...
const Navbar = () => {
    ...
    const navLinkStyles = ({ isActive }) => 
        `font-semibold text-sm transition-colors duration-200 hover:text-blue-600 ${
    ...
    return (
        <nav className='w-full h-20 px-4 sm:px-8 md:px-10 bg-white border-b border-slate-100 flex flex-row justify-between items-center z-50 fixed top-0 left-0 right-0'>
            ...
            >
                <GraduationCap size={40} className="text-blue-600 transition-transform group-hover:scale-105" />
            ...
            {/* Desktop Action Action Buttons */}
            <div className='hidden md:flex flex-row gap-3 items-center relative' ref={dropdownRef}>
                
                {/* Staff / Management Dropdown Trigger */}
                <button 
                    onClick={() => setShowStaffDropdown(!showStaffDropdown)} 
                    className={`flex flex-row gap-1.5 items-center px-4 py-2 border font-semibold rounded-xl transition-all duration-200 text-sm cursor-pointer ${
                        showStaffDropdown 
                ...
                {/* Management Nested Popover Menu */}
                {showStaffDropdown && (
